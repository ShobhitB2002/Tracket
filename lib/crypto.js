...
// Key for /api/cron, derived so no extra setting is needed (admin panel shows the URL).
const cronKey = () => crypto.createHmac('sha256', subkey('cron')).update('cron').digest('hex').slice(0, 32);

module.exports = { randomToken, friendlyPassword, hashPassword, verifyPassword, safeEqual, sign, verify, encrypt, decrypt, sha, cronKey };
