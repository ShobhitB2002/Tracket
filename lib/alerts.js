...
async function evaluate(ctx, { tz, state, hb, source } = {}) {
  ...
  const onLeave = await require('./leave').on(ctx.uid, day);
  const shift = onLeave != null ? null : shiftFor(await store.get(K(ctx.uid, 'shift'), { cache: 60000 }), day, tz);
  ...
  if (run && p.nudge.late && onLeave == null && (shift ? now >= shift.to + 15 * MIN : now >= core.atTimeIn(day, '21:00', tz))) {
  ...
  if (p.report.monthly) await require('./report').maybeSendMonthly(ctx, { tz, now, day }).catch(() => {});
  await require('./slack').sync(ctx, run).catch(() => {}); // backup; /api/event syncs at once
}
...
async function autoStep(ctx, { tz, state, autonow }) {
  ...
  if (!busy) {
    ...
    if (hasAuto) {
      ...
      if (p.auto.failsafe && run && !A[`fs${run.startedAt}`] && (await require('./leave').on(ctx.uid, day)) == null) {
...
async function command(ctx, { action, gid } = {}) {
  ...
  if (action === 'stop') {
    ...
    if (p.auto.failsafe && (await require('./leave').on(uid, day)) == null) {
