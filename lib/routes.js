...
// The ticket a plain Start would use (see core.nextTicket), cached for maxAge ms.
async function nextCached(ctx, tz, maxAge) {
  const k = `u:${ctx.uid}:next`;
  const hit = await store.get(k);
  if (hit && Date.now() - hit.at < maxAge) return hit.next;
  const next = await core.nextTicket(ctx, tz).catch(() => hit?.next || null);
  await store.set(k, { at: Date.now(), next }, 3600);
  return next;
}
...
const WIDGET = path.join(__dirname, '..', 'tracket.widget.js');
...
const routes = {
  ...
  'GET /api/health': ['public', async () => {
    ...
    return {
      ...
      imported: storeInfo.imported || null,
      import_error: storeInfo.importError || null,
  ...
  'POST /api/event': ['member', async ({ who, body }) => {
    ...
    const before = pre[`u:${ctx.uid}:state`]?.running?.startedAt;
    ...
    if (state.running?.startedAt !== before) await quietly(slack.sync(ctx, state.running));
    let autonow = pre[`u:${ctx.uid}:autonow`];
    const step = await alerts.autoStep(ctx, { tz: body.tz, state, autonow }).catch((e) => { console.error('[auto]', e.message); return {}; });
    if (step.offered) autonow = step.offered;
    await quietly(alerts.evaluate(ctx, { tz: body.tz, state, source: 'event' }));
    return { ok: true, auto: await alerts.current(ctx.uid, autonow || null), beatIn: step.beatIn || undefined, now: Date.now() };
  }],
  'POST /api/auto': ['member', async ({ who, body }) => alerts.autoOp(await memberCtx(who), body)],
  // The Start / Stop button (dashboard, menu bar, ⌘K): body { action: 'start'|'stop', gid? }
  'POST /api/control': ['member', async ({ who, body }) => alerts.command(await memberCtx(who), body)],
  // What a plain Start would pick: the last logged ticket that isn't complete
  'GET /api/next': ['member', async ({ who, query }) => ({ next: await nextCached(await memberCtx(who), core.tzOf({ tz: query.tz }), query.fresh ? 0 : 60000) })],
  // leave & holidays: GET → { days }, POST { add: { from, to, label } } | { remove: date|[dates] }
  'GET /api/leave': ['member', async ({ who }) => leave.get(who.member.id)],
  'POST /api/leave': ['member', async ({ who, body }) => leave.update(who.member.id, body)],
  // Slack status
  'GET /api/slack': ['member', async ({ who }) => ({ connected: !!(await slack.info(who.member.id)), ...((await slack.info(who.member.id)) || {}) })],
  'POST /api/slack': ['member', async ({ who, body }) => slack.connect(who.member.id, body.token)],
  'DELETE /api/slack': ['member', async ({ who }) => slack.disconnect(who.member.id)],
  'POST /api/slack/test': ['member', async ({ who }) => slack.test(who.member.id)],
  // Timesheet for any range (≤ 62 days): ?from&to&format=csv|json
  'GET /api/timesheet': ['member', async ({ who, query }) => {
    const ctx = await memberCtx(who);
    const t = await core.timesheet(ctx, { tz: query.tz, from: query.from, to: query.to });
    if (query.format !== 'csv') return t;
    const q = (v) => `"${String(v ?? '').replace(/"/g, '""')}"`;
    const lines = [['Date', 'Ticket', 'Project', 'Hours', 'Minutes', 'Link'].join(',')];
    for (const r of t.rows) lines.push([r.date, q(r.name), q(r.project), (r.minutes / 60).toFixed(2), r.minutes, q(r.url)].join(','));
    lines.push(['Total', '', '', (t.total / 60).toFixed(2), t.total, ''].join(','));
    return { raw: '\ufeff' + lines.join('\r\n'), type: 'text/csv; charset=utf-8', filename: `tracket-timesheet-${t.from}-to-${t.to}.csv` };
  }],
  'GET /api/insights': ['member', async ({ who, query }) => core.insights(await memberCtx(who), { tz: query.tz, range: query.range === 'month' ? 'month' : 'week' })],

  'GET /api/history': ['member', async ({ who, query }) => core.history(await memberCtx(who), { tz: query.tz, days: query.days })],
  ...
  // ---- daily email report ----
  'POST /api/report/monthly': ['member', async ({ who, query, req }) => {
    await users.limit(req, 'report', 6, 3600);
    const ctx = await memberCtx(who);
    const tz = core.tzOf({ tz: query.tz });
    return require('./report').sendMonthly(ctx, core.ymdIn(Date.now(), tz).slice(0, 7), tz, { siteUrl: siteUrl(req), toDate: true });
  }],
  'POST /api/report/weekly': ['member', async ({ who, query, req }) => {
    await users.limit(req, 'report', 6, 3600);
    const ctx = await memberCtx(who);
    const tz = core.tzOf({ tz: query.tz });
    return require('./report').sendWeekly(ctx, core.ymdIn(Date.now(), tz), tz, { siteUrl: siteUrl(req) });
  }],
  ...
  'GET /api/bar': ['member', async ({ who, query, req }) => {
    ...
    const g = await store.mget([u('hb'), u('state'), u(`day:${day}`), u('alerts'), u('autonow')]);
    ...
    // act=1: this menu bar can drive the browser's Asana tabs, so it may run
    // auto mode too (frozen background tabs can't)
    let autonow = g[u('autonow')];
    if (query.act === '1') {
      const step = await alerts.autoStep(ctx, { tz, state: s, autonow }).catch((e) => { console.error('[auto]', e.message); return {}; });
      if (step.offered) autonow = step.offered;
    }
    ...
    return {
      ...
      cap: p.auto.cap.on ? p.auto.cap.hours : null,
      next: run ? null : await nextCached(ctx, tz, 300000),
      auto: await alerts.current(ctx.uid, autonow || null),
    };
  }],
  // The Scriptable (iPhone/iPad) home-screen widget with this member's URL + key.
  'GET /api/widget': ['member', async ({ who, req }) => {
  ...
  'GET /api/userscript': ['member', async ({ who, req }) => {
    ...
    const src = fs.readFileSync(USERSCRIPT, 'utf8')
      ...
      // also runs on the dashboard, to relay Start/Stop to the Asana tabs instantly
      .replace(/^\/\/ @match        https:\/\/app\.asana\.com\/\*$/m, `// @match        https://app.asana.com/*\n// @match        ${siteUrl(req)}/*`)
      .replace(/^const TRACKET_URL = .*$/m, `const TRACKET_URL = ${JSON.stringify(siteUrl(req))};`)
  ...
  'GET /api/admin/usage': ['admin', async () => usage.report()],
  // Members who opted in (Settings → Account): status and totals only, never tickets.
  'GET /api/admin/team': ['admin', async () => {
    const members = Object.values((await store.get('members')) || {}).filter((m) => m.status === 'active');
    const now = Date.now(), out = [];
    for (const m of members) {
      const p = await prefs.get(m.id);
      if (!p.team.share) continue;
      const tz = core.tzOf({ tz: p.tz });
      const days = [...Array(7)].map((_, i) => core.ymdIn(now - i * 864e5, tz));
      const u = (n) => `u:${m.id}:${n}`;
      const g = await store.mget([u('state'), u('hb'), ...days.map((d) => u(`day:${d}`))]);
      const run = g[u('state')]?.running;
      const runToday = run && core.ymdIn(run.startedAt, tz) === days[0] ? run : null;
      const liveMin = runToday ? Math.max(0, (now - runToday.startedAt) / 60000) : 0;
      const today = (g[u(`day:${days[0]}`)]?.totalMinutes || 0) + liveMin;
      const week = days.reduce((a, d) => a + (g[u(`day:${d}`)]?.totalMinutes || 0), 0) + liveMin;
      const away = await leave.on(m.id, days[0]);
      out.push({ id: m.id, name: m.name, leave: away, running: !!runToday, since: runToday?.startedAt || null,
        linked: !!g[u('hb')] && now - g[u('hb')].at < core.FRESH_MS, today: Math.round(today), week: Math.round(week) });
    }
    out.sort((a, b) => b.today - a.today);
    return { team: out, at: now };
  }],
...
async function dispatch(args) {
  usage.add('requests');
  try { return await run(args); } finally { await usage.flush(); }
}
async function run({ method, pathname, query, body, req }) {
...
module.exports = { dispatch, _routes: routes }; // _routes: for tests
