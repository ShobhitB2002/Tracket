...
const routes = {
  ...
  'GET /api/health': ['public', async () => {
    ...
    return {
      ...
      email_reports: has('RESEND_API_KEY'),
      email_from_own_domain: has('RESEND_FROM'),
  ...
  'GET /api/bar': ['member', async ({ who, query, req }) => {
    ...
    const hb = await store.get(`u:${ctx.uid}:hb`);
    const tz = core.tzOf({ tz: query.tz || p.tz || hb?.tz });
    const now = Date.now(), day = core.ymdIn(now, tz);
    const s = await core.getState(ctx);
    let data = await store.get(`u:${ctx.uid}:day:${day}`);
    if ((!data || now - data.fetchedAt > 60000) && (await store.lock(`u:${ctx.uid}:poll`, 8000))) data = await core.buildDay(ctx, day, tz, true).catch(() => data);
    await quietly(alerts.evaluate(ctx, { tz, state: s, hb, source: 'bar' }));
    const f = await alerts.feed(ctx.uid);
  ...
  'GET /api/menubar': ['member', async ({ who, req }) => {
    ...
    return { raw: src, type: 'text/javascript; charset=utf-8', filename: 'tracket.10s.js' };
  }],
  ...
  'POST /api/cron': ['public', async ({ req, query }) => {
    ...
    return { ok: true, ...(await everyMember((ctx) => alerts.evaluate(ctx, { source: 'cron' }))) };
  }],
