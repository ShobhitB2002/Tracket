...
async function onReport(ctx, { running, previous, visible, diag, tz }) {
  ...
  if (running?.taskGid && running.startedAt) {
    ...
    if (!ended && (!cur || start >= cur.startedAt)) {
      ...
      s.running = r;
      if (!cur || cur.startedAt !== start) await rememberTask(ctx, ymdIn(start, tz), gid, r.taskName, r.url);
    }
...
async function today(ctx, query) {
  ...
  return { ...data, running: day === todayStr ? runningToday(s.running, tz) : null, lastHeartbeat: hb?.at || 0, serverNow: Date.now(), today: todayStr };
}
...
async function live(ctx, query) {
  ...
  const hb = await store.get(K(ctx, 'hb'));
  return { running: runningToday(s.running, tz), lastHeartbeat: hb?.at || 0, serverNow: Date.now(), today: todayStr, sig };
}
...
module.exports = { today, live, onReport, clearRunning, verifyPat, _internal: { getState, ymdIn, dayStartMs } };
