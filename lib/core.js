...
async function live(ctx, query) {
  ...
  const hb = await store.get(K(ctx, 'hb'));
  return { running: await withCheck(ctx, runningToday(s.running, tz)), lastHeartbeat: hb?.at || 0, scriptV: hb?.diag?.v || null, serverNow: Date.now(), today: todayStr, sig, _state: s, _hb: hb };
}
...
module.exports = {
  today, live, onReport, clearRunning, verifyPat, removeLunch, settleSuspect,
