...
async function rangeTotals(ctx, from, to, tz) {
  ...
  if (!complete) {
    ...
    for (const d of days) {
      ...
      for (const t of v.tasks) { const x = byTask.get(t.gid) || { gid: t.gid, name: t.name, url: t.url, minutes: 0 }; x.minutes += t.minutes; byTask.set(t.gid, x); }
    }
  }
  return { tasks: [...byTask.values()].sort((a, b) => b.minutes - a.minutes), byDay, complete };
}
...
module.exports = {
  ticketMeta, ticketInfo, nextTicket, rangeTotals, insights,
