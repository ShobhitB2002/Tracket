...
// cached for 30s per warm instance: read on every request, changed rarely
const getMembers = async () => (await store.get('members', { cache: 30000 })) || {};
...
const getKeys = async () => (await store.get('apikeys', { cache: 30000 })) || {};
...
async function touch(id) {
  if (store.throttle(`seen:${id}`, 5 * 60000)) await store.set(`u:${id}:seen`, now());
}
...
// Changes the login email (also where reports go). Needs the current password.
async function changeEmail(id, email, password) {
  email = normEmail(email);
  if (!validEmail(email)) throw bad(400, 'Enter a valid email');
  const members = await getMembers();
  const m = members[id];
  if (!m || !c.verifyPassword(password, m.pw)) throw bad(401, 'Password is wrong');
  if (Object.values(members).some((x) => x.id !== id && x.email === email)) throw bad(409, 'Another member already uses that email');
  m.email = email;
  await putMembers(members);
  return email;
}
...
async function removeMember(id) {
  ...
  for (const k of ['pat', 'state', 'hb', 'me', 'names', 'seen', 'shift', 'chk', 'prefs', 'push', 'alerts', 'autonow', 'lunchfix', 'reportlast']) await store.del(`u:${id}:${k}`);
}
...
module.exports = {
  ...
  createMember, resetPassword, changePassword, changeEmail, setStatus, removeMember, rotateApiKey, apiKeyOf,
