...
// Resolves the caller: { role: 'admin' } | { role: 'member', member } | { role: 'guest' }
async function resolve(req) {
  const key = req.headers['x-tracket-key'];
  if (key) {
    const id = (await getKeys())[c.sha(key)];
    const m = id && (await getMembers())[id];
    if (m && m.status === 'active') return { role: 'member', member: m, via: 'key' };
    return { role: 'guest', badKey: true };
  }
  const s = c.verify(readCookie(req, MEMBER_COOKIE));
  if (s?.uid) {
    const m = (await getMembers())[s.uid];
    if (m && m.status === 'active' && m.sv === s.sv) return { role: 'member', member: m, via: 'cookie' };
  }
  const a = c.verify(readCookie(req, ADMIN_COOKIE));
  if (a?.admin && a.stamp === adminStamp() && process.env.ADMIN_PASSWORD) return { role: 'admin' };
  return { role: 'guest' };
}
...
async function removeMember(id) {
  ...
  for (const k of ['pat', 'state', 'hb', 'me', 'names', 'seen']) await store.del(`u:${id}:${k}`);
}
...
const logoutCookies = () => [cookie(MEMBER_COOKIE, '', 0), cookie(ADMIN_COOKIE, '', 0)];
...
async function decide(id, approve) {
  ...
  if (r.status !== 'pending') throw bad(409, `Already ${r.status}`);
  let created = null;
...
module.exports = {
  ...
  addRequest, decide, overview, setAsanaConnected, bad,
};
