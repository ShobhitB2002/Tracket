...
//   report  — { on, weekly, monthly } end-of-day, end-of-week and monthly email reports
//   slack   — { on, emoji } Slack status while a timer runs (token stored separately, see lib/slack.js)
...
const DEFAULTS = { tz: null, lunch: null, auto: { cap: { on: false, hours: 7 }, lunch: false, failsafe: false }, report: { on: false, weekly: false, monthly: false }, slack: { on: false, emoji: ':large_green_circle:' },
...
async function update(uid, input = {}) {
  ...
  if (input.report) {
    ...
    if ('monthly' in input.report) cur.report.monthly = !!input.report.monthly;
  }
  ...
  if (input.slack) {
    if ('on' in input.slack) cur.slack.on = !!input.slack.on;
    if (typeof input.slack.emoji === 'string' && /^:[a-z0-9_+-]{1,40}:$/.test(input.slack.emoji)) cur.slack.emoji = input.slack.emoji;
  }
