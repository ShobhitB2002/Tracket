...
function ready() {
  ...
  if (!readyP) readyP = (async () => {
    ...
    if (IMPORT_FROM) await importTurso().catch((e) => console.error('[store] Turso import skipped:', e.message));
...
const info = async () => { if (backend === 'turso') await ready(); return { backend, migrated, imported, region: backend === 'turso' ? (TURSO_URL.match(/\.(aws-[a-z0-9-]+|[a-z]{3})\.turso\.io/) || [])[1] || null : null }; };
