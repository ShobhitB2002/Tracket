...
function ready() {
  ...
  if (!readyP) readyP = (async () => { await sql([{ sql: SCHEMA }]); if (hasRedis) await migrate(); })().catch((e) => { readyP = null; throw e; });
  return readyP;
}
...
async function migrate() {
  const done = await sql([{ sql: "SELECT v FROM kv WHERE k = '__migrated'" }]);
  if (done[0].rows.length) { migrated = JSON.parse(done[0].rows[0][0]); return; }
  const now = Date.now();
  ...
  migrated = { at: Date.now(), copied };
  await sql([{ sql: "INSERT INTO kv (k, v, exp) VALUES ('__migrated', ?, NULL) ON CONFLICT(k) DO UPDATE SET v = excluded.v", args: [JSON.stringify(migrated)] }]);
}
...
const info = async () => { if (backend === 'turso') await ready(); return { backend, migrated }; };
