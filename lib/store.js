...
function ready() {
  ...
  if (!readyP) readyP = (async () => {
    ...
    if (IMPORT_FROM) await importTurso().catch((e) => { importError = e.message; console.error('[store] Turso import skipped:', e.message); });
...
// One-time copy from another Turso database in the same group (e.g. a
// per-deployment branch), set by TRACKET_IMPORT_FROM=<host>. Only runs while
// this database has no members, so it can never overwrite live data.
const IMPORT_FROM = (process.env.TRACKET_IMPORT_FROM || '').trim().replace(/^(libsql|https):\/\//, '').replace(/\/+$/, '');
let imported = null, importError = null;
async function importTurso() {
  const have = await sql([{ sql: "SELECT k, v FROM kv WHERE k IN ('__imported', 'members')" }]);
  const row = (k) => have[0].rows.find((r) => r[0] === k);
  if (row('__imported')) { imported = JSON.parse(row('__imported')[1]); return; }
  if (row('members')) return;
  const res = await fetch(`https://${IMPORT_FROM}/v2/pipeline`, {
    method: 'POST',
    // the source's own token (a branch has its own); falls back to ours
    headers: { Authorization: `Bearer ${(process.env.TRACKET_IMPORT_TOKEN || '').trim() || TURSO_TOKEN}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ requests: [{ type: 'execute', stmt: { sql: 'SELECT k, v, exp FROM kv' } }, { type: 'close' }] }),
  });
  const body = await res.json().catch(() => ({}));
  if (!res.ok || body.results?.[0]?.type !== 'ok') throw new Error(`source answered ${res.status} ${JSON.stringify(body).slice(0, 160)}`);
  const rows = body.results[0].response.result.rows.map((r) => r.map(cell)).filter(([k]) => !k.startsWith('lock:') && !k.startsWith('rl:'));
  const stmts = rows.map(([k, v, exp]) => ({ sql: 'INSERT INTO kv (k, v, exp) VALUES (?, ?, ?) ON CONFLICT(k) DO UPDATE SET v = excluded.v, exp = excluded.exp', args: [k, v, exp] }));
  for (let i = 0; i < stmts.length; i += 100) await sql(stmts.slice(i, i + 100));
  imported = { at: Date.now(), copied: stmts.length, from: IMPORT_FROM.split('.')[0] };
  await sql([{ sql: "INSERT INTO kv (k, v, exp) VALUES ('__imported', ?, NULL) ON CONFLICT(k) DO NOTHING", args: [JSON.stringify(imported)] }]);
}

// ---------- local file ----------
const FILE = process.env.TRACKET_DATA_FILE || path.join(__dirname, '..', 'data', 'store.json'); // tests use their own
let mem = null;
function load() {
...
// ---------- API ----------
// Rough operation counts for the admin's usage page (lib/usage.js flushes them).
...
// Number of live keys (health check).
async function count() {
  if (backend !== 'turso') return null;
  const [r] = await t([{ sql: 'SELECT COUNT(*) FROM kv WHERE exp IS NULL OR exp > ?', args: [Date.now()] }]);
  return r.rows[0][0];
}

const info = async () => { if (backend === 'turso') await ready(); return { backend, migrated, imported, importError: IMPORT_FROM && !imported ? importError || 'not run' : null, region: backend === 'turso' ? (TURSO_URL.match(/\.(aws-[a-z0-9-]+|[a-z]{3})\.turso\.io/) || [])[1] || null : null }; };

module.exports = { get, mget, set, del, incr, lock, throttle, sweep, info, count, hosted, backend, ops };
